...
function createForm() {
    ...
    const hpInput = newInput("number")
        .setId("totalHP")
        .setMin(MIN_HP)
        .setMax(MAX_HP)
        .defaultVal(MIN_HP * 4)
        .setStep(25)
        .setPlaceholder("Total Health")
        .isRequired(true);
    ...
    const dmgBox = newElem("section")
        .setClass("baseDmg");
    label = mkLabel("Base Damage:");

    minInput = newInput("number")
        .setId("minDmg")
        .setMin(MIN_DMG)
        .setMax(MAX_DMG)
        .setStep(2)
        .defaultVal(MIN_DMG)
        .setPlaceholder("Min");
    maxInput = newInput("number")
        .setId("maxDmg")
        .setMin(MIN_DMG + 1)
        .setMax(MAX_DMG + 10)
        .setStep(2)
        .defaultVal(MIN_SPDMG)
        .setPlaceholder("Max");
    inner = newElem("section")
        .addLast(minInput, maxInput);
    ...
    minInput = newInput("number")
        ...
        .setStep(5)
        .defaultVal(MIN_SPDMG)
        .setPlaceholder("Sp. Min");
    maxInput = newInput("number")
        .setId("maxSpDmg")
        .setMin(MIN_SPDMG + 1)
        .setMax(MAX_SPDMG + 10)
        .setStep(5)
        .defaultVal(MIN_SPDMG * 2)
        .setPlaceholder("Sp. Max");
    inner = newElem("section")
        .addLast(minInput, maxInput);

    const specDmgBox = newElem("section")
        .setClass("spDmg")
        .addLast(label, inner);

    // Button Section
    const bttn = newElem("button")
        .setId("newChar")
        .insideTxt("Create Char");
    ...
    // Set event listeners to all Inputs
...
function addNewForm(e) {
    ...
    const _nC = () => {
        ...
        if (PLAYERS.length < MAX_PLAYERS) {
            let addPlayerBttn = newElem("button")
                .setId("addPlayer")
                .insideTxt("New Slayer");
...
function mkLabel(_innerTxt) {
    return newElem("label")
        .insideTxt(_innerTxt);
}

function startGameButton() {
    if (document.querySelector("#startGameBttn") === null) {
        let bttn = newElem("button")
            .setId("startGameBttn")
            .insideTxt("Attack Monster");
...
function init() {
    ...
    let startBttn = newElem("button")
        .setId("startBttn")
        .insideTxt("Start");
    // startBttn.addEventListener("click", newCharacter);
...
let promise = new Promise(function () {
    playerTurnOver(() => resolve("All players have acted"), true);
});
